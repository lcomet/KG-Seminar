...
In general terms, traditional clustering techniques are used in a variety of scenarios and application areas, as for example data analysis and interpretation \cite{Pedrycz}. Those algorithms (such as k-means type partitional algorithms, and hierarchical algorithms) focused on clustering, pattern mining, and classification can be extended to graphs representations \cite{Aggarwal}. In this respect, there are different types of clustering algorithms, which are classified in a variety of taxonomies \cite{Zacharski} \cite{Pedrycz} \cite{Berkhin}). But regardless the classification, those algorithms mainly consider the similarity and/or distance among entities \cite{Pedrycz}. There are many studies covering surveys on the different techniques used for graph clustering, for instance: \cite{Schaeffer}, \cite{Aggarwal}, \cite{Carpineto}. 

In this context, Aggarwal et al. \cite{Aggarwal} indicate that clustering algorithms can be grouped in two big categories: node clustering, which tries to identify dense areas on the graph based on edge behaviour by using functions to determine distances between data points or a similarity value to create cluster of nodes; and structural clustering which attempts to group multiple graphs based on their structural behaviour.



The main differences on clustering documents vs clustering search results, according to Carpineto et al. \cite{Carpineto} are: meaningful labels using natural language used in search results vs centroids in traditional algorithms, cluster computation is done online in search results vs offline in document clustering, input data in search results is shorter (snippets) in search results vs the whole document used when clustering documents, the number of clusters in a traditional algorithm is fixed while in search results this number is variable, in search results may exist overlapping clusters but using traditional methods the clusters are disjoint, in Web search results allows using GUI while document clustering has not this option.

The changes to do to conventional clustering algorithms to adapt to graphs \cite{Aggarwal}: adapting the measures to define similarity/distances between structural objects, improving it. Also they mention that the use of centroids in k-means is good for multi-dimensional objects but needs to be adapted for graphs to create representative objects. 

\cite{Aggarwal} find that typical problems on graph clustering are related to node and graph clustering, such as pseudo-cliques (high probability that an edge exists between any pair of nodes), and shingles (sub-graphs having a large number of common links). Also, when large number of graphs need to be clustered based on their underlying structural behaviour problems may arise when structures do not match.
In the context of the Web, Carpineto et al. \cite{Carpineto} point out that clustering after information retrieval is more efficient, and probably more effective than applying it before.

\cite{Carpineto} distinguish three categories of algorithms: data-centric, which are traditional data clustering algorithms applied to clustering search results but it has problems with labeling the output in a more human readable format; description-aware, which are algorithms introducing clusters' descriptions interpretable by humans; and description-centric, which are algorithms special for clustering search results and which combine clusters' quality and cluster's descriptions.

\cite{Carpineto} Several factors contribute to the overall computational performance of a search results clustering engine. The most critical tasks involve the first three components: search result acquisition, preprocessing, and clustering.

Carpineto et al. \cite{Carpineto} conclude that clustering in search results improve semantics between the user's request and the results returned by a plain search engine.

Chang et al. \cite{Chang} indicate that search result clustering techniques do not create clusters with good quality because of the complexity on unsupervised clustering. Search results are constantly changing therefore requires dynamic clustering process. Also they indicate that clusters should include descriptive cluster labels and be semantically coherent.
