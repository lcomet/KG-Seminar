...
Different tasks and purposes can be served by clustering including: i) Exploring underlying structure of data, ii) Discovering meaningful patterns, and iii) Summarising key characteristics of data. \cite{Elbattah}
...
Elbattah et al. \cite{Elbattah}, in the context of a large scale KG environment, propose a technique which focuses on clustering entities with similar structure. They suggest that is possible to divide entities in a KG into groups forming communities. Hence, those entities are very similar when considering their linked-based structure.

In order to test their hypothesis, they use a subset of knowledge base of Freebase. In this context, they take into account only four different categories of entities (Science \& Technology, Society, Sports, and Time \& Space), and only the ``Is-A" relation among those entities. In this sense, they point out how the algorithm can influence the clustering results in terms of scalability and quality of resulting clusters. For this, they review clustering algorithms, selecting the Louvain algorithm for performing clustering on the Freebase subset.
They highlight that Louvain algorithm is optimisation-based, therefore less computational demanding. 

The fundamental idea of the Louvain algorithm is the division of each node into a individual community (i.e. the number of communities is equal to the number of nodes) after that calculating the gain in modularity by placing the node into a different community, if the gain is good, then the node is moved to that community. After, a new graph is created with the communities detected in the previous step, and this two steps are repeated until the maximum modularity is reached.

After the clustering, they find that some of the computed clusters were part of larger categories. Moreover, some clusters present a pronounced cross-category structure, meaning that one entity could be part of more that one cluster/category. 

They conclude that the computed clusters are good, based on graph modularity and cluster density. The clusters are then used to get insights and reveal unknown relationships among entities and with this finding new interpretations for the data.
