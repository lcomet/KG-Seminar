...
Saeedi et al. \cite{Saeedi} emphasize that problems may arise in large-scale environments in link discovery when identifying multiple representations referring to the same entity. Therefore, they suggest using clustering when more than two sources are needed, and with this merging all the matches of the same entity resulting in a single entity representation of the KG. In this respect, they first refer to a similarity graph\footnote{``A graph in which vertices represent entities and edges are links between matching entities. There is no direct link between entities of the same source. Edges have a property for the similarity value (real number in the interval [0,1]) indicating the degree of similarity."\cite{Saeedi}} which is built by FAMER, a previous framework developed by Saeedi et al. for distributed multi-source entity resolution (e.g. linking schemes, links from the Web of Data) \cite{Peukert}. Then, the clustering schemes\footnote{These schemes are: 1. Connected components, 2. Center clustering, 3. Merge Center, 4. Start clustering (two variations), 5. Correlation clustering. \cite{Peukert}} use the similarity graph to resolve entity matching and creates clusters maximizing similarity among entities inside a cluster, and minimizing similarity among entities in different groups. Although the proposed clustering technique seems valid, Saeedi et al. discover some issues in the resulting clusters (e.g. overlapping clusters, source-inconsistent clusters when entities have more than one entity per source). To overcome those issues, Saeedi et al. propose CLIP algorithm, another clustering technique based on link priority to detect clusters with exempt of overlapping and inconsistencies. This new approach additionally to the graph similarity, takes into account link strength and link degree. Moreover, the authors also propose RLIP, as an approach for repairing clusters by eliminating overlap clusters.

As the similarity graph may also consists of links with low similiarity values, the core idea of CLIP algorithm is to prioritize strong links when clustering entities, and ignore weak links\footnote{``A link is strong if it is the maximum link from both sides, i.e. for ei to source B and for ej to source A. On the other hand, a link is weak if it is not the maximum link for any of the two sides."\cite{Saeedi}}. The input of the algorithm is a similarity graph which is used to determine the clusters. In the first step, the algorithm determines the strength of all links and applies \textit{ConnectedComponents} scheme to identify complete clusters, while considering only entities with strong links among them. Then, in the second step also includes normal links\footnote{``A link is consider to be normal if it is the maximum link for only one of the two sides."\cite{Saeedi}}. This process is done iteratively and in parallel, and the output is the resulting cluster set including all the source-consistent clusters. An example of this process is shown in \ref{fig1}, where green lines represent strong links, red dashed lines indicate weak links, and blue lines represent normal links. The output in Phase 1 consists of a cluster with four connected components, because those posses strong links among them. Moreover, in Phase 2, the complete clusters\footnote{``A source-consistent cluster that contains entities from all sources is called a complete cluster."\cite{Saeedi}}. are not consider anymore, thus the connected component is calculated for the input and cluster to entities of the same sources already present in the formed cluster are removed (\ref{fig1} after preprocessing).

Moreover, RLIP algorithm is used to repair the output of different clustering algorithms. In its first step resolves overlapping clusters by also given priority to strong links in intra-cluster links among entities to select the cluster to which an overlapped entity should be assigned. Here, the algorithm considers the highest association degree\footnote{``The association degree of an entity for a cluster of size k corresponds to the average similarity of the entity to the k − 1 other entities in the cluster."\cite{Saeedi}} when overlapped entities have strong links to multiple clusters. Besides, also considers overlapped entities with no strong links as singletons. Also, when an entity is strongly linked to only overlapped entities, all of them become singletons. \cite{Saeedi}
Then, uses CLIP algorithm to repair source-incosistent clusters.

\begin{center}
\includegraphics[width=1\textwidth]{clip_example.png} \label{fig1}
\captionof{figure}{Example of CLIP \cite{Saeedi}}
\end{center}

Saeedi et al. \cite{Saeedi} point out that CLIP surpass the other clustering schemes which they studied, particularly in precision and F-measure (test's accuracy) for all the tested datasets, although the recall is lightly poor comparing to \textit{ConnectedComponents}. This is mainly because CLIP algorithm does not consider weak links. Moreover, they indicates that RLIP algorithm can improve F-measure for all clustering schemes, which demonstrates exceptional effectiveness of this approach of cluster repair. Besides, the repaired results of \textit{ConnectedComponents} are identical to the CLIP results, while the clusters determined with CCPivot and Center can be improved in a minor way because these algorithms already remove many links. The authors also indicates that the CLIP component 
