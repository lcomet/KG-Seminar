...
Schmitz et al. \cite{Schmitz} focus on applying clustering in the context of semantic peer-to-peers systems. More specifically, in a peer-to-peer network with personal knowledge management (P2PPKM) systems. A P2PPKM system consists on a peer-to-peer network where every peer creates and share its own personal knowledge base, and that have a similar function. Moreover, they refer to the network as being self-organized, which means that the network uses routing indices (i.e. each peer stores a view of the content of its neighbors, thus it can decide to which peer derived a query or answer).

Furthermore, what Schmitz et al. \cite{Schmitz} identify as a problem in this type of systems is the lack of expertise or ``semantic self-descriptions" shared among peers (i.e. every peer publishes their whole content but not their most specific knowledge). Hence, they introduce a new technique for knowledge clustering to be applied in P2PPKM systems, which only uses a set of the total knowledge base, containing fewer entities. They indicates that the value of having the expertise information of each peers is that the routing algorithm can know what information can find in the knowledge base of a specific peer.

Moreover, in \cite{Schmitz} they use the concept of ontologies. Hence, each peer share their ontologies (e.g. metadata related to bibliographic information: BibTEX ontology, and a classification scheme). Furthermore, they use the concept of shortest path to measure semantic distance between nodes in the graph.
...
To yield max- imum flexibility and to use as much of the modeled content as possible, an edge counting approach was chosen for this paper.

To overcome these problems (Pitfalls on Real-World Ontologies, Noise and Technical Artifacts, Modeling Idiosyncrasies), we have implemented extensive entity filtering and weighting customization strategies which are applied prior to the metric computation itself.
we will use an extension of k-modes clustering [11] to obtain ag- gregations of knowledge bases.
The basic version of k-modes clustering for par- titioning a set S of items into k clusters S1,...,Sk such that S = �� ̇ iSi works as follows:
1. Given k, choose k elements Ci,i = 1...k of S as centroids
2. Assign each s ∈ S to the cluster Si with i = argminj d(Cj,s)
3. For i = 1 . . . k, recompute Ci such that �� d(Ci, s) is minimized. s∈Si
