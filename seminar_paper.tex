...
\textbf{Algorithm} & \textbf{Advantages} & \textbf{Issues}\\
 ...
 \hline
 CLIP (\ref{entity-clustering}) & - Good quality in resulting clusters; - Overlap-free and source-consistent clusters ; - Better precision and accuracy performance & Slow execution time when iteratively processing source-inconsistent components \\
 \hline
 ...
 \hline
 Knowledge-graph-based clustering (\ref{app}) & - Supports meta-data information; - Maps topics to concept in existing knowledge bases; - Clusters' ranking computation; - Includes interactive user interface; - Includes semantic labels; - Allows dynamic clustering & Performance may be an issue when working with large KG \\
 \hline
\end{tabular}

In Table \ref{table2} we sum up the main advantages and detected issues mentioned by the authors of the analyzed papers. Here we can notice that the main issue in the algorithms is still related to scalability, as performance is reduce in large KG.
