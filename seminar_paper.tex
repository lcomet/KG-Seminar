...
Schmitz et al. \cite{Schmitz} focus on applying clustering in a peer-to-peer network with personal knowledge management (P2PPKM) systems. A P2PPKM system consists on a peer-to-peer network which has a similarity function, and where every peer creates and shares its own personal knowledge base. These networks are self-organized, which means that the network uses indices to choose one route instead of another (i.e. each peer stores a view of the content of its neighbors, thus it can decide to which peer derived a query or answer).

However the concept of P2PPKM systems helps in sharing knowledge among peers in the network, Schmitz et al. \cite{Schmitz} identify the lack of expertise or ``semantic self-descriptions" shared among them (i.e. every peer publishes their whole content but not their most specific knowledge which in turn generates inefficiencies such as excessive traffic in the network). As a consequence, they propose a new technique for knowledge clustering in P2PPKM systems, but focusing on the notion of semantic systems, which consists on using only a set of the total knowledge base (i.e. containing fewer entities). 

Moreover, in their proposed solution, each peer share their ontologies which are domain specific (e.g. metadata related to bibliographic information: BibTEX ontology, and a classification scheme), and they measure the semantic distance between nodes in a graph by using shortest path algorithm. Furthermore, for the content aggregation of the knowledge bases, they use bi-section k-modes clustering which is an extension of the original k-modes clustering algorithm for the extraction of expertise. As described in \cite{Huang}, the k-modes technique extends the k-means algorithm, but instead of using mean values for clustering uses mode values. Besides, this algorithm calculates dissimilarity among categorical entities, and update the value of modes based on frequency, to minimize the clustering cost.

In the specific implementation in \cite{Schmitz}, the bi-section k-modes clustering, in the first step generates one cluster consisting of all nodes in the graph. Then, it divides the cluster with the largest variance with 2-modes, the algorithm do this in a recursively way until k clusters are reached. Moreover, instead of defining the value of k beforehand, the authors use "silhouette coefficient"\footnote{"The silhouette coefficient is an indicator for the quality of the clustering. It determines how well clusters are separated in terms of the distances of each item to the nearest and the second nearest centroid: if each item is close to its own centroid and far away from the others, the silhouette coefficient will be large, indicating a good clustering."\cite{Schmitz}}. What is also important to mention, is that the centroids in the graph consists only of the ontology shared among peers (i.e. expertise), which it makes it more specific and understandable for the peers. 

Schmitz et al. \cite{Schmitz} test their proposed technique of expertise extraction in a self-organized P2PKM network with data related to scientists and bibliographic information of their publications. This environment possess few entities as only authors who wrote papers on at least 10 topics are considered. Their findings after applying the suggested approach, indicates that the size of the expert knowledge shared among peers influences the information that can be find, meaning that a bigger size of knowledge base will contain more information than a smaller one. Also, the k-modes clustering technique performs better than other approaches with regards to number of queries needed to get a specific information, this because of indices used in the information transmission. Moreover, using the proposed clustering technique helps to extract the appropriate expertise in a PMPKM environment.
However, it is important to remark that in large ontologies scalability issues may arise, due to computation of shortest path.
